...
/**
 * Scheme based on a commonly used idea: can be extremely fast. It encodes
 * integers in blocks of 128 integers. For arrays containing an arbitrary number
 * of integers, you should use it in conjunction with another CODEC:
 * 
 * <pre>
 * IntegerCODEC ic = 
...
public final class BinaryPacking implements IntegerCODEC
{
	        ...
	        IntWrapper outpos)
	{
		...
		for (int s = inpos.get(); s < inpos.get() + inlength; s += 32 * 4)
		{
			...
			tmpoutpos += mbits2;
			BitPacking.fastpackwithoutmask(in, s + 2 * 32, out, tmpoutpos,
			        mbits3);
			tmpoutpos += mbits3;
			BitPacking.fastpackwithoutmask(in, s + 3 * 32, out, tmpoutpos,
			        mbits4);
			tmpoutpos += mbits4;
		}
		inpos.add(inlength);
