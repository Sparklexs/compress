...
public final class Simple9 implements IntegerCODEC
{
	        ...
	        IntWrapper outpos)
	{
		...
		// 最后一组剩余28个数字，但压缩的个数与之前不同，所以单独考虑
	...
	{
		IntegerCODEC s9 = new Simple9();
		int[] in = { 1, 1, 1, 1, 1, 1, 1, 1, 1 };
		IntWrapper inpos = new IntWrapper(0);
		IntWrapper outpos = new IntWrapper(0);
		int[] out = new int[50];
		s9.compress(in, inpos, in.length, out, outpos);
		int decomp[] = new int[in.length];
		int inlength = outpos.intValue();
		outpos.set(0);
		inpos.set(0);
		s9.uncompress(out, inpos, inlength, decomp, outpos);
