...
public final class DeltaZigzagBinaryPacking implements IntegerCODEC
{
	        ...
	        int[] outBuf, IntWrapper outPos)
	{
		...
		for (; ip < inPosLast; ip += BLOCK_LENGTH)
		{
			...
			outBuf[op++] = (bits1 << 24) | (bits2 << 16) | (bits3 << 8)
			        | (bits4 << 0);
	...
	private static int pack(int[] inBuf, int inOff, int[] outBuf, int outOff,
	        int validBits)
	{
	...
	private static int unpack(int[] inBuf, int inOff, int[] outBuf, int outOff,
	        int validBits)
	{
