 ...
 * </p>
 * 
 * You should only use this scheme on sorted arrays. Use FastPFOR if you have
 * unsorted arrays.
 * 
...
public final class IntegratedFastPFOR implements IntegratedIntegerCODEC
{
	...
	int	             pageSize;
	// 相对于FastPFOR多了一组int[128]，它的作用是用来暂存原始输入delta后的gap数组
	        ...
	        int[] out, IntWrapper outpos, IntWrapper initoffset)
	{
		...
		{
			initoffset.set(Delta.delta(constin, tmpconstinpos, BLOCK_SIZE,
			        initoffset.intValue(), buffer));
	        ...
	        IntWrapper outpos, int thissize, IntWrapper initoffset)
	{
		...
		for (int run = 0, run_end = thissize / BLOCK_SIZE; run < run_end; ++run, tmpoutpos += BLOCK_SIZE)
		{
			...
			initoffset.set(Delta.fastinverseDelta(out, tmpoutpos, BLOCK_SIZE,
			        initoffset.intValue()));
		}
		outpos.set(tmpoutpos);
		inpos.set(inexcept);
