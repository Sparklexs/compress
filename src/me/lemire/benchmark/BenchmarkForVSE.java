...
import me.lemire.compression.Composition;
...
public class BenchmarkForVSE
{
	        ...
	        throws IOException
	{
		...
		for (int r = 0; r < repeat; ++r)
		{
			...
			for (int k = 0; k < N; ++k)
			{
				...
				inpos.set(0);
				outpos.set(0);
				decompressBuffer = new int[data[k].length];
				...
				for (int m = 0; m < outpos.get(); ++m)
				{
					...
					{
						/*
						 * 自己设置的输出当前所有生成序列 if (debackup[m] != backupdata[m])
						 * System.err.println("s"); FileWriter dataWriter = new
						 * FileWriter("data"); FileWriter backWriter = new
						 * FileWriter("delta"); FileWriter decomWriter = new
						 * FileWriter("decompress"); for (int i = 0; i <
						 * outpos.get(); i++) { dataWriter
						 * .write(Integer.toString(data[k][i]) + ",");
						 * backWriter.write(Integer.toString(backupdata[i]) +
						 * ","); decomWriter
						 * .write((Integer.toString(debackup[i]) + ",")); }
						 * dataWriter.flush(); dataWriter.close();
						 * backWriter.flush(); backWriter.close();
						 * decomWriter.flush(); decomWriter.close();
						 */
		...
		if (verbose)
		{
			// size统计所有从0到N中的元素被压缩后的int大小
			// totalSize则是压缩之前的数组length之和
			double bitsPerInt = size * 32.0 / totalSize;
			double compressSpeed = totalSize * repeat * 1.0 / (compressTime);
			double decompressSpeed = totalSize * repeat * 1.0
			        / (decompressTime);
			System.out.println(String.format("\t%1$.2f\t%2$f\t%3$f",
			        bitsPerInt, compressSpeed, decompressSpeed));
			csvLog.format("\"%1$s\",%2$d,%3$.2f,%4$f,%5$f\n", c.toString(),
	        ...
	        throws IOException
	{
		...
		for (int sparsity = 1; sparsity < max_sparsity; ++sparsity)
		{
			...
			// 预跑两遍
			testCodec(csvLog, sparsity, new CAFOR(), data, repeat, false);
			testCodec(csvLog, sparsity, new CAFOR(), data, repeat, false);

			// 开始记录结果
			testCodec(csvLog, sparsity, new CAFOR(), data, repeat, true);
			System.out.println();
		}
	}
}
