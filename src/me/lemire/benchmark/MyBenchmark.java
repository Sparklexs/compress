...
import me.lemire.compression.ByteIntegerCODEC;
import me.lemire.compression.Delta;
import me.lemire.compression.IntWrapper;
import me.lemire.compression.IntegerCODEC;
import me.lemire.compression.IntegratedByteIntegerCODEC;
import me.lemire.compression.IntegratedIntegerCODEC;
import me.lemire.compression.OptPFDS16;
import me.lemire.compression.SimpleE2;
...
public class MyBenchmark
{
	        ...
	        throws IOException
	{
		if (verbose)
		{
			...
			System.out.println("# bits per int, compress speed (mis), "
			        + "decompression speed (mis) ");
		}
		...
		for (int r = 0; r < repeat; ++r)
		{
			...
			for (int k = 0; k < N; ++k)
			{
				...
				int[] debackup = Arrays.copyOf(decompressBuffer,
				        decompressBuffer.length);
				if (!(c instanceof IntegratedIntegerCODEC))
				...
				for (int m = 0; m < outpos.get(); ++m)
				{
					...
					{
						if (debackup[m] != backupdata[m])
							System.err.println("s");
						FileWriter dataWriter = new FileWriter("data");
						FileWriter backWriter=new FileWriter("delta");
						FileWriter decomWriter=new FileWriter("decompress");
						for (int i = 0; i < outpos.get(); i++)
						{
							dataWriter.write(Integer.toString(data[k][i]) + ",");
							backWriter.write(Integer.toString(backupdata[i]) + ",");
							decomWriter.write((Integer.toString(debackup[i]) + ","));
						}
						dataWriter.flush();
						dataWriter.close();
						backWriter.flush();
	...
	public static void main(String args[]) throws IOException
	{
		...
		System.out.println("# 	 Softw. Pract. Exper.40, 2 (February 2010), "
		        + "131-147. ");
		System.out.println();
	...
	private static void test(PrintWriter csvLog, int N, int nbr, int repeat)
	        throws IOException
	{
		csvLog.format("\"Algorithm\",\"Sparsity\",\"Bits per int\","
		...
		for (int sparsity = 1; sparsity < max_sparsity; ++sparsity)
		{
			...
			testCodec(csvLog, sparsity, new OptPFDS16(), data, repeat, false);
			testCodec(csvLog, sparsity, new OptPFDS16(), data, repeat, false);
			testCodec(csvLog, sparsity, new OptPFDS16(), data, repeat, true);
			System.out.println();
			...
			// System.out.println();
			//
			// testByteCodec(csvLog, sparsity, new VariableByte(), data, repeat,
			// false);
			// testByteCodec(csvLog, sparsity, new VariableByte(), data, repeat,
			// false);
			// testByteCodec(csvLog, sparsity, new VariableByte(), data, repeat,
			...
			// System.out.println();
			//
			// testByteCodec(csvLog, sparsity, new IntegratedVariableByte(),
			// data,
			// repeat, false);
			// testByteCodec(csvLog, sparsity, new IntegratedVariableByte(),
			// data,
			...
			// System.out.println();
			//
			// testCodec(csvLog, sparsity, new Composition(new BinaryPacking(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new BinaryPacking(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new BinaryPacking(),
			// new VariableByte()), data, repeat, true);
			// System.out.println();
			//
			// testCodec(csvLog, sparsity, new Composition(new NewPFD(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new NewPFD(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new NewPFD(),
			...
			// System.out.println();
			//
			// testCodec(csvLog, sparsity, new Composition(new NewPFDS9(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new NewPFDS9(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new NewPFDS9(),
			...
			// System.out.println();
			//
			// testCodec(csvLog, sparsity, new Composition(new NewPFDS16(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new NewPFDS16(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new NewPFDS16(),
			...
			// System.out.println();
			//
			// testCodec(csvLog, sparsity, new Composition(new OptPFD(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new OptPFD(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new OptPFD(),
			...
			// System.out.println();
			//
			// testCodec(csvLog, sparsity, new Composition(new OptPFDS9(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new OptPFDS9(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new OptPFDS9(),
			...
			// System.out.println();
			//
			// testCodec(csvLog, sparsity, new Composition(new OptPFDS16(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new OptPFDS16(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new OptPFDS16(),
			...
			// }
			//
			// testCodec(csvLog, sparsity, new Composition(new FastPFOR(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new FastPFOR(),
			// new VariableByte()), data, repeat, false);
			// testCodec(csvLog, sparsity, new Composition(new FastPFOR(),
			...
			//
			// {
			// IntegerCODEC c = new Composition(new XorBinaryPacking(),
			// new VariableByte());
			// testCodec(csvLog, sparsity, c, data, repeat, false);
			// testCodec(csvLog, sparsity, c, data, repeat, false);
			// testCodec(csvLog, sparsity, c, data, repeat, true);
