...
public class PForDelta
{
	...
	private static final int[]	MASK	    = { 0x00000000, 0x00000001,
	...
	/**
	 * Compress one block of blockSize integers using PForDelta with the optimal
	 * parameter b
	 * 
	 * @param inBlock
	        ...
	        int blockSize)
	{
		...
		{
			compressedBits = decompressBBitSlots(outBlock, inBlock, blockSize,
			        bits);
		...
		if (expNum > 0)
		{
			compressedBits = decompressBlockByS16(expAux, inBlock, offset,
			        expNum * 2);
			offset += compressedBits;
	 ...
	 *            the block to be compressed
	 * @param bits
	 *            the numbers of bits to decide whether a number is a big number
	 * @param blockSize
	 *            the block size
	        ...
	        int blockSize) throws IllegalArgumentException
	{
		...
		for (int i = 0; i < blockSize; ++i)
		{
			...
			// exp
			{
				...
				writeBits(tmpCompressedBlock, inputBlock[i] & MASK[bits],
				        outputOffset, bits);
		...
		if (expNum > 0)
		{
			int compressedBitSize = compressBlockByS16(tmpCompressedBlock,
			        outputOffset, expAux, expNum * 2);
	        ...
	        int outStartOffsetInBits, int[] inBlock, int blockSize)
	{
		...
		{
			num = Simple16.s16Compress(outCompBlock, outOffset, inBlock,
			        inOffset, numLeft, blockSize);
			outOffset++;
			inOffset += num;
		}
	 ...
	 * @param bits
	 *            the number of bits to be read, unlike writeBits(), readBits()
	 *            does not deal with bits==0 and thus bits must > 0. When bits
	 *            ==0, the calling functions will just skip the entire bits-bit
	 *            slots without decoding them
